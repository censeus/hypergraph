...
def create_metrics_store(config: "MetricsConfig", id: str) -> MetricsStore:
    ...
    if config.writer:
        from hypergraph_llm.metrics.metrics_writer_factory import create_metrics_writer
    ...
    if strategy not in metrics_store_factory:
        match strategy:
            case MetricsStoreType.Memory:
                from hypergraph_llm.metrics.memory_metrics_store import MemoryMetricsStore
