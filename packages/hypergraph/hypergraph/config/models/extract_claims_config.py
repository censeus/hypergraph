...
class ExtractClaimsConfig(BaseModel):
    ...
    enabled: bool = Field(
        ...
        default=hypergraph_config_defaults.extract_claims.enabled,
    )
    completion_model_id: str = Field(
        description="The model ID to use for claim extraction.",
        default=hypergraph_config_defaults.extract_claims.completion_model_id,
    )
    model_instance_name: str = Field(
        ...
        default=hypergraph_config_defaults.extract_claims.model_instance_name,
    )
    prompt: str | None = Field(
        description="The claim extraction prompt to use.",
        default=hypergraph_config_defaults.extract_claims.prompt,
    )
    description: str = Field(
        description="The claim description to use.",
        default=hypergraph_config_defaults.extract_claims.description,
    )
    max_gleanings: int = Field(
        description="The maximum number of entity gleanings to use.",
        default=hypergraph_config_defaults.extract_claims.max_gleanings,
    )
