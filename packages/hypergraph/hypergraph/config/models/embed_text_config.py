...
class EmbedTextConfig(BaseModel):
    ...
    embedding_model_id: str = Field(
        ...
        default=hypergraph_config_defaults.embed_text.embedding_model_id,
    )
    model_instance_name: str = Field(
        ...
        default=hypergraph_config_defaults.embed_text.model_instance_name,
    )
    batch_size: int = Field(
        description="The batch size to use.",
        default=hypergraph_config_defaults.embed_text.batch_size,
    )
    batch_max_tokens: int = Field(
        description="The batch max tokens to use.",
        default=hypergraph_config_defaults.embed_text.batch_max_tokens,
    )
    names: list[str] = Field(
        description="The specific embeddings to perform.",
        default=hypergraph_config_defaults.embed_text.names,
    )
