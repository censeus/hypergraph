...
from hypergraph_storage.tables.table_provider_config import TableProviderConfig
from hypergraph_vectors import IndexSchema, VectorStoreConfig, VectorStoreType
from pydantic import BaseModel, Field, model_validator

from hypergraph.config.defaults import hypergraph_config_defaults
from hypergraph.config.embeddings import all_embeddings
from hypergraph.config.enums import AsyncType, ReportingType
...
class HyperGraphConfig(BaseModel):
    ...
    def _validate_input_base_dir(self) -> None:
        ...
        if self.input_storage.type == StorageType.File:
            if not self.input_storage.base_dir:
                msg = "input storage base directory is required for file input storage. Please rerun `hypergraph init` and set the input storage configuration."
    ...
    chunking: ChunkingConfig = Field(
        ...
        default=ChunkingConfig(
            ...
            prepend_metadata=hypergraph_config_defaults.chunking.prepend_metadata,
        ),
    )
    ...
    def _validate_output_base_dir(self) -> None:
        ...
        if self.output_storage.type == StorageType.File:
            if not self.output_storage.base_dir:
                msg = "output base directory is required for file output. Please rerun `hypergraph init` and set the output configuration."
                raise ValueError(msg)
    ...
    update_output_storage: StorageConfig = Field(
        ...
        default=StorageConfig(
            base_dir=hypergraph_config_defaults.update_output_storage.base_dir,
        ),
    )
    ...
    def _validate_update_output_storage_base_dir(self) -> None:
        ...
        if self.update_output_storage.type == StorageType.File:
            if not self.update_output_storage.base_dir:
                msg = "update_output_storage base directory is required for file output. Please rerun `hypergraph init` and set the update_output_storage configuration."
                raise ValueError(msg)
    ...
    cache: CacheConfig = Field(
        ...
        default=CacheConfig(**asdict(hypergraph_config_defaults.cache)),
    )
    ...
    def _validate_reporting_base_dir(self) -> None:
        ...
        if self.reporting.type == ReportingType.file:
            if self.reporting.base_dir.strip() == "":
                msg = "Reporting base directory is required for file reporting. Please rerun `hypergraph init` and set the reporting configuration."
                raise ValueError(msg)
    ...
    workflows: list[str] | None = Field(
        ...
        default=hypergraph_config_defaults.workflows,
    )
    ...
    extract_claims: ExtractClaimsConfig = Field(
        ...
        default=ExtractClaimsConfig(
            enabled=hypergraph_config_defaults.extract_claims.enabled,
        ),
    )
    ...
    def _validate_vector_store(self) -> None:
        """Validate the vector store configuration specifically in the Hypergraph context. This checks and sets required dynamic defaults for the embeddings we require."""
    ...
    def _validate_vector_store_db_uri(self) -> None:
        ...
        if store.type == VectorStoreType.LanceDB:
            if not store.db_uri or store.db_uri.strip() == "":
                store.db_uri = hypergraph_config_defaults.vector_store.db_uri
    ...
    def get_completion_model_config(self, model_id: str) -> ModelConfig:
        ...
        if model_id not in self.completion_models:
            err_msg = f"Model ID {model_id} not found in completion_models. Please rerun `hypergraph init` and set the completion_models configuration."
    ...
    def get_embedding_model_config(self, model_id: str) -> ModelConfig:
        ...
        if model_id not in self.embedding_models:
            err_msg = f"Model ID {model_id} not found in embedding_models. Please rerun `hypergraph init` and set the embedding_models configuration."
            raise ValueError(err_msg)
