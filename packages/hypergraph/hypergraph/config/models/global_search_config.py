...
class GlobalSearchConfig(BaseModel):
    ...
    map_prompt: str | None = Field(
        ...
        default=hypergraph_config_defaults.global_search.map_prompt,
    )
    reduce_prompt: str | None = Field(
        description="The global search reducer to use.",
        default=hypergraph_config_defaults.global_search.reduce_prompt,
    )
    completion_model_id: str = Field(
        description="The model ID to use for global search.",
        default=hypergraph_config_defaults.global_search.completion_model_id,
    )
    knowledge_prompt: str | None = Field(
        description="The global search general prompt to use.",
        default=hypergraph_config_defaults.global_search.knowledge_prompt,
    )
    max_context_tokens: int = Field(
        description="The maximum context size in tokens.",
        default=hypergraph_config_defaults.global_search.max_context_tokens,
    )
    data_max_tokens: int = Field(
        description="The data llm maximum tokens.",
        default=hypergraph_config_defaults.global_search.data_max_tokens,
    )
    map_max_length: int = Field(
        description="The map llm maximum response length in words.",
        default=hypergraph_config_defaults.global_search.map_max_length,
    )
    reduce_max_length: int = Field(
        description="The reduce llm maximum response length in words.",
        default=hypergraph_config_defaults.global_search.reduce_max_length,
    )
    ...
    dynamic_search_max_level: int = Field(
        ...
        default=hypergraph_config_defaults.global_search.dynamic_search_max_level,
    )
