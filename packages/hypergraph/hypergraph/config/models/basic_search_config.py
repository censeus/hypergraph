...
class BasicSearchConfig(BaseModel):
    ...
    prompt: str | None = Field(
        ...
        default=hypergraph_config_defaults.basic_search.prompt,
    )
    completion_model_id: str = Field(
        description="The model ID to use for basic search.",
        default=hypergraph_config_defaults.basic_search.completion_model_id,
    )
    embedding_model_id: str = Field(
        description="The model ID to use for text embeddings.",
        default=hypergraph_config_defaults.basic_search.embedding_model_id,
    )
    k: int = Field(
        description="The number of text units to include in search context.",
        default=hypergraph_config_defaults.basic_search.k,
    )
    max_context_tokens: int = Field(
        description="The maximum tokens.",
        default=hypergraph_config_defaults.basic_search.max_context_tokens,
    )
