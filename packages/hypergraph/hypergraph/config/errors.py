...
class ApiKeyMissingError(ValueError):
    ...
    def __init__(self, llm_type: str, azure_auth_type: str | None = None) -> None:
        ...
        msg += ". Please rerun `hypergraph init` and set the API_KEY."
        super().__init__(msg)
...
class AzureApiBaseMissingError(ValueError):
    ...
    def __init__(self, llm_type: str) -> None:
        """Init method definition."""
        msg = f"API Base is required for {llm_type}. Please rerun `hypergraph init` and set the api_base."
        super().__init__(msg)
...
class AzureApiVersionMissingError(ValueError):
    ...
    def __init__(self, llm_type: str) -> None:
        """Init method definition."""
        msg = f"API Version is required for {llm_type}. Please rerun `hypergraph init` and set the api_version."
        super().__init__(msg)
