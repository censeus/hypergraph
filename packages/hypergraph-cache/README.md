...
This package contains a collection of utilities to handle Hypergraph caching implementation.

### Basic

This example shows how to create a JSON cache with file storage using the Hypergraph cache package's configuration system. 

[Open the notebook to explore the basic example code](example-notebooks/basic_cache_example.ipynb)

### Custom Cache

This example demonstrates how to create a custom cache implementation by extending the base Cache class and registering it with the Hypergraph cache system. Once registered, the custom cache can be instantiated through the factory pattern using either CacheConfig or directly via cache_factory, allowing for extensible caching solutions tailored to specific needs.
