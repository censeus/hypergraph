...
⚠️ Hypergraph can consume a lot of LLM resources! We strongly recommend starting with the tutorial dataset here until you understand how the system works, and consider experimenting with fast/inexpensive models first before committing to a big indexing job.

## Requirements

[Python 3.10-3.12](https://www.python.org/downloads/)

The following is a simple end-to-end example for using Hypergraph on the command line after installing from [pypi](https://pypi.org/project/hypergraph/).

It shows how to use the system to index some text, and then use the indexed data to answer questions about the documents.

## Install Hypergraph

To get started, create a project space and python virtual environment to install `hypergraph`.

### Create Project Space

```bash
mkdir hypergraph_quickstart
cd hypergraph_quickstart
python -m venv .venv
```
...
```bash
python -m pip install hypergraph
```

### Initialize Hypergraph

To initialize your workspace, first run the `hypergraph init` command.
...
- `input` Location of text files to process with `hypergraph`.
- `.env` contains the environment variables required to run the Hypergraph pipeline. If you inspect the file, you'll see a single environment variable defined,
  `HYPERGRAPH_API_KEY=<API_KEY>`. Replace `<API_KEY>` with your own OpenAI or Azure API key.
...
If running in OpenAI mode, you only need to update the value of `HYPERGRAPH_API_KEY` in the `.env` file with your OpenAI API key.
...
```sh
hypergraph query "What are the top themes in this story?"
```
...
- For more details about configuring Hypergraph, see the [configuration documentation](config/overview.md).
