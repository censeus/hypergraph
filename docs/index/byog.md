...
Several users have asked if they can bring their own existing graph and have it summarized for query with Hypergraph. There are many possible ways to do this, but here we'll describe a simple method that aligns with the existing Hypergraph workflows quite easily.

To cover the basic use cases for Hypergraph query, you should have two or three tables derived from your data:
...
The approach described here will be to run a custom Hypergraph workflow pipeline that assumes the text chunking, entity extraction, and relationship extraction has already occurred.
...
Hypergraph includes the ability to specify *only* the specific workflow steps that you need. For basic graph summarization and query, you need the following config in your settings.yaml:

```yaml
workflows: [create_communities, create_community_reports]
```

This will result in only the minimal workflows required for Hypergraph [Global Search](../query/global_search.md).
...
[FastHypergraph](./methods.md#fasthypergraph) uses text_units for the community reports instead of the entity and relationship descriptions. If your graph is sourced in such a way that it does not have descriptions, this might be a useful alternative. In this case, you would update your workflows list to include the text variant of the community reports workflow:
...
- Run `hypergraph index --root <your_project_root>`