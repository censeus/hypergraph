...
def _make_mock_model(response_text: str) -> AsyncMock:
    ...
    model.completion_async = AsyncMock(return_value=result)
    return model
...
async def test_llm_failure_graceful(
    ...
    model = AsyncMock()
    model.completion_async = AsyncMock(side_effect=Exception("LLM unavailable"))
...
async def test_single_entity_skips():
    ...
    model.completion_async.assert_not_called()
    assert list(result_entities["title"]) == ["Only One"]


@pytest.mark.asyncio
async def test_all_entities_processed(mock_callbacks):
    """All entities are sent to the LLM in a single call."""
    ...
    # Model should have been called once (single call for all entities)
    assert model.completion_async.call_count == 1
